...
function mattata:process_plugin_extras()
    ...
    if not mattata.is_plugin_disabled('pole', message) and message.chat.type ~= 'private' and (message.text:match('^'..mattata.case_insensitive_pattern('pole')) or message.text:match('^'..mattata.case_insensitive_pattern('subpole')) or ('^'..mattata.case_insensitive_pattern('fail'))) then
        ...
        if message.text:match('^'..mattata.case_insensitive_pattern('pole')) and not mattata.is_pole_done(message.chat.id, message.from.id) then
            ...
            redis:hset('pole_stats:' .. message.chat.id, message.from.id, (redis:hget('pole_stats:' .. message.chat.id, message.from.id) or 0)+1)
            redis:hset('group_pole_stats:' .. message.chat.id, message.from.id, (redis:hget('group_pole_stats:' .. message.chat.id, message.from.id) or 0)+5)
            redis:hset('global_pole', message.from.id, (redis:hget('global_pole', message.from.id) or 0)+1)
            redis:hset('global_pole_stats', message.from.id, (redis:hget('global_pole_stats', message.from.id) or 0)+5)
        ...
        elseif message.text:match('^'..mattata.case_insensitive_pattern('subpole')) and not mattata.is_subpole_done(message.chat.id, message.from.id) then
            ...
            redis:hset('subpole_stats:' .. message.chat.id, message.from.id, (redis:hget('subpole_stats:' .. message.chat.id, message.from.id) or 0)+1)
            redis:hset('group_pole_stats:' .. message.chat.id, message.from.id, (redis:hget('group_pole_stats:' .. message.chat.id, message.from.id) or 0)+3)
            redis:hset('global_subpole', message.from.id, (redis:hget('global_subpole', message.from.id) or 0)+1)
            redis:hset('global_pole_stats', message.from.id, (redis:hget('global_pole_stats', message.from.id) or 0)+3)
        ...
        elseif message.text:match('^'..mattata.case_insensitive_pattern('fail')) and not mattata.is_fail_done(message.chat.id, message.from.id) then
            ...
            redis:hset('group_pole_stats:' .. message.chat.id, message.from.id, (redis:hget('group_pole_stats:' .. message.chat.id, message.from.id) or 0)+1)
            redis:hset('global_fail', message.from.id, (redis:hget('global_fail', message.from.id) or 0)+1)
            redis:hset('global_pole_stats', message.from.id, (redis:hget('global_pole_stats', message.from.id) or 0)+1)
