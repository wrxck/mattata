        ...
        if msg_count == tonumber(msg_limit) and not mattata.is_global_admin(message.from.id) then -- Don't run the antispam plugin if the user is configured as a global admin in `configuration.lua`.
            ...
            elseif mattata.get_setting(message.chat.id, 'log administrative actions') then
                mattata.send_message(
                    ...
                    string.format(
                        '#action #antispam #admin_'..api.info.id..' #user_'..message.from.id..' #group_'..message.chat.id:gsub("%-", "")..'\n\n<pre>' .. language['antispam']['6'] .. '</pre>',
                ...
                if mattata.get_setting(message.chat.id, 'notify admins actions') then
                    for i, admin in pairs(mattata.get_chat_administrators(message.chat.id).result) do
                      mattata.send_message(
                          admin.user.id,
                          string.format(
                              '#action #antibot #admin_'..api.info.id..' #user_'..message.from.id..' #group_'..message.chat.id:gsub("%-", "")..'\n\n<pre>' .. language['antispam']['6'] .. '</pre>',
                              mattata.escape_html(api.info.first_name),
                              api.info.id,
                              mattata.escape_html(message.from.first_name),
                              message.from.id,
                            ...
                            if mattata.get_setting(message.chat.id, 'notify admins actions') then
                                for i, admin in pairs(mattata.get_chat_administrators(message.chat.id).result) do
                                  mattata.send_message(
                                      admin.user.id,
                                      string.format(
                                          '#action #wordfilter #admin_'..api.info.id..' #user_'..message.from.id..' #group_'..message.chat.id:gsub("%-", "")..'\n\n<pre>' .. language['antispam']['6'] .. '</pre>',
                                          mattata.escape_html(api.info.first_name),
                                          api.info.id,
                                          mattata.escape_html(message.from.first_name),
                                          message.from.id,
              ...
              if mattata.get_setting(message.chat.id, 'notify admins actions') then
                  for i, admin in pairs(mattata.get_chat_administrators(message.chat.id).result) do
                    mattata.send_message(
                        admin.user.id,
                        string.format(
                            '#action #antilink #admin_'..api.info.id..' #user_'..message.from.id..' #group_'..message.chat.id:gsub("%-", "")..'\n\n<pre>' .. language['antispam']['6'] .. '</pre>',
                            mattata.escape_html(api.info.first_name),
                            api.info.id,
                            mattata.escape_html(message.from.first_name),
                            message.from.id,
