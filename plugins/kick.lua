...
function kick:init(configuration)
	...
	kick.help = configuration.commandPrefix .. 'kick - Kicks the replied-to user. Forwarded messages are treated as if the user it was forwarded from had sent the message.'
end
						...
						if message.reply_to_message.forward_from then
							if message.reply_to_message.forward_from.id ~= self.info.id and admin.user.id ~= message.reply_to_message.forward_from.id then
								...
								if not res then
									...
									mattata.sendMessage(configuration.kickLog, '<pre>' .. mattata.htmlEscape('Kicked ' .. message.reply_to_message.forward_from.first_name .. ' (' .. message.reply_to_message.forward_from.id .. ') from ' .. message.chat.title .. ' (' .. message.chat.id .. ')') .. '</pre>', 'HTML', true, false)
									return
								end
							end
						else
							...
							if not res then
								...
								mattata.sendMessage(configuration.kickLog, '<pre>' .. mattata.htmlEscape('Kicked ' .. message.reply_to_message.from.first_name .. ' (' .. message.reply_to_message.from.id .. ') from ' .. message.chat.title .. ' (' .. message.chat.id .. ')') .. '</pre>', 'HTML', true, false)
								return
							end
						end
					end
