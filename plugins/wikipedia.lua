...
function wikipedia:init(configuration)
 ...
 wikipedia.doc = configuration.command_prefix .. [[wikipedia <query>
Returns an article from Wikipedia.
Aliases: ]] .. configuration.command_prefix .. 'w, ' .. configuration.command_prefix .. 'wiki'
end
...
function wikipedia:action(msg, configuration)
 ...
 jstr, res = HTTP.request(search_url .. URL.escape(input))
 if res ~= 200 then
 ...
 local res_url = 'http://en.wikipedia.org/w/api.php?action=query&prop=extracts&format=json&exchars=4000&exsectionformat=plain&titles='
 jstr, res = HTTP.request(res_url .. URL.escape(title))
 if res ~= 200 then
  functions.send_reply(self, msg, configuration.errors.connection)
  return
 end
 ...
 local url = 'http://en.wikipedia.org/wiki/' .. URL.escape(title)
 title = title:gsub('%(.+%)', '')
 local output
