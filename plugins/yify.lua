...
function yify.getResultCount(input)
	...
	if not jdat.data.movies then return false end
...
function yify.getResult(input, result)
	...
	if res ~= 200 then return false, nil end
	local jdat = json.decode(jstr)
	if not jdat.data.movies then return false, nil end
	local buttons = {}
	for n = 1, #jdat.data.movies[result].torrents do table.insert(buttons, { text = jdat.data.movies[result].torrents[n].quality, url = jdat.data.movies[result].torrents[n].url }) end
	local keyboard = {}
	...
	if description:len() > 500 then description = description:sub(1, 500) .. '...' end
...
function yify:onMessage(message, configuration, language)
	...
	if not input then mattata.sendMessage(message.chat.id, yify.help, nil, true, false, message.message_id) return end
	...
	local pages = {
		{ text = '◀️', callback_data = 'yify:results:0' },
		{ text =  '1/' .. yify.getResultCount(input), callback_data = 'yify:pages:1:' .. yify.getResultCount(input) },
		{ text = '▶️', callback_data = 'yify:results:2' }
	}
	table.insert(keyboard.inline_keyboard, pages)
