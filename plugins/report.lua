...
function report:init()
    report.commands = mattata.commands(self.info.username):command('report').table
    report.help = '/report - Reports the replied-to message to all administrators of the group, via private message.'
end
...
function report:on_message(message, configuration, language)
    ...
    local notified = 0
    for n in pairs(admins.result)
    do
        if admins.result[n].user.id ~= self.info.id
        then
            ...
            then -- If it's a public supergroup, then we'll link to the report message to make
            -- things easier for the administrator the report was sent to.
