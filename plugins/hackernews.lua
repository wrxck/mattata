...
local function get_hackernews_results(configuration)
	...
	for i = 1, 8 do
		...
		if ires ~= 200 then
			mattata.sendMessage(msg, configuration.errors.connection, nil, true, false, msg.message_id, nil)
			return
		end
...
function hackernews:onMessageReceive(msg, configuration)
	...
	mattata.sendMessage(msg.chat.id, output, 'HTML', true, false, msg.message_id, nil)
end
