...
function quotes:init()
    quotes.commands = mattata.commands(
        self.info.username
    ):command('quotes').table
...
function quotes:on_callback_query(callback_query, message, configuration)
    ...
    if callback_type == 'delete_all' then
        if redis:get('quotes:' .. callback_query.from.id) then
    ...
    elseif callback_type == 'delete' then
        if not redis:get('quotes:' .. callback_query.from.id) then
            return mattata.answer_callback_query(
                callback_query.id,
                'All quotes saved under your database entry have already been deleted!'
            )
