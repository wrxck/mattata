...
function get:init(configuration)
	...
	get.commands = mattata.commands(self.info.username, configuration.commandPrefix):c('get').table
...
function get:getValue(message, variable)
	...
	if hash then
		...
		if not value then
			return 'There is currently no saved value for \'' .. variable .. '\'.'
		else
...
function get:listVariables(message)
	...
	if hash then
		...
		if text == '' or text == nil then
			return 'No variables have been given values in this chat!'
		else
			return text
		end
	end
...
function get:onMessage(message)
	...
	local output
	if input then
		output = get:getValue(message, input:match('(.+)'))
	else
		output = get:listVariables(message)
	end
