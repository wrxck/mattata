...
function help:onQueryReceive(callback, message)
	...
	if callback.data == 'helpStatistics' then
		...
		mattata.editMessageText(message.chat.id, message.message_id, helpStatistics .. '\n\nYou can view message statistics in groups I\'m part of, using the command \'' .. configuration.commandPrefix .. 'statistics\'.', nil, true, '{"inline_keyboard":[[{"text":"Back", "callback_data":"helpBack"}]]}')
	end
...
function help:onMessageReceive(message)
	...
	if input then
		...
		mattata.sendMessage(message.chat.id, 'I\'m sorry, but I\'m afraid there is no help documented for that plugin at this moment in time. If you believe this is a mistake, please don\'t hesitate to contact [my developer](https://telegram.me/wrxck).', nil, true, false, message.message_id, nil)
		return
	end
