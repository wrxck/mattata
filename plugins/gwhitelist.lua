...
function gwhitelist:on_message(message, configuration)
    ...
    elseif message.reply_to_message then
        ...
        redis:del(hash)
        local output = message.from.first_name .. ' [' .. message.from.id .. '] has whitelisted ' .. message.reply_to_message.from.first_name .. ' [' .. message.reply_to_message.from.id .. '] to use ' .. self.info.first_name ..'.'
        if input then
            output = output .. '\nReason: ' .. input
        end
