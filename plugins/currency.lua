...
function currency:init(configuration)
	...
	currency.doc = configuration.command_prefix .. 'currency (amount) <from> TO <to> - Returns exchange rates for various currencies. Source: Google Finance.'
end
function currency:action(msg, configuration)
	local input = msg.text:upper()
	if not input:match('%a%a%a TO %a%a%a') then
		functions.send_reply(msg, currency.doc, true)
		return
	end
