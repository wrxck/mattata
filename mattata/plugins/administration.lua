...
local drua = require('mattata.drua-tg')
			...
			for _, user in ipairs(utilities.index(input)) do
				...
				elseif user:match('^@') then
					...
					if target then
						...
						table.insert(t, { err = 'Sorry, I do not recognise that username ('..user..').' })
					end
				else
...
function administration.init_command(self_, config_)
	administration.commands = {
...

		{
			...
			action = function(self, msg, _, config)
				...
				for _, chat_id_str in ipairs(self.database.administration.activity) do
					...
					if (not group.flags[1]) and group.link then
		...
		{
			triggers = utilities.triggers(self_.info.username, config_.cmd_pat):t('url').table,

			privilege = 1,
			interior = true,

			action = function(self, msg, group, config)
				local output = 'No url has been set for ' .. msg.chat.title .. '.'
				if group.link then
					output = '' .. group.link .. ''
				end
				utilities.send_message(self, msg.chat.id, output, true, nil, true)
